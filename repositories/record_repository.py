...
def create_user_record(db: Session, record: record_schema.RecordCreate):
    ...
    db_item.face_score = (db_item.anger_score + db_item.scorn_score + db_item.disgust_score + db_item.happy_score + db_item.neutral_score + db_item.sad_score + db_item.surprised_score) / 7
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
    return db_item
